# Backend

I like working with data — building things that move, store, transform, and serve it correctly under load.

What I build most:

* REST APIs
* Queue and worker-based systems — this shows up in most of my larger projects
* Data-heavy backends: collection, processing, pipelines
* Systems designed with automation, high load and optimization in mind

Auth systems can be part of a project, but they're not where I'd say my strength is — if a project needs something auth-heavy, I'm upfront about that going in.

I'm not attached to a specific stack. Go and Python are my primary tools, with PostgreSQL, SQLite and Redis depending on what the data actually needs.